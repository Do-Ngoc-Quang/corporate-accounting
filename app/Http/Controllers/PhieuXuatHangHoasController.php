...
class PhieuXuatHangHoasController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if (!$exist_PhieuXuatHangHoa) {
            ...
            // ---
            $SoLuongTon = DanhMucHangHoa::where('MaHang', $request->MaHang)->value('SoLuongTonDau');
            if ((int)$request->SoLuong - (int)$SoLuongTon > 0) {
    ...
    public function phieuxuathanghoachitiet_update(Request $request, $id)
    {
        ...
        if ($phieuxuathanghoachitiet) {
            ...
            // ---
            $SoLuong_in_PXHHCT = PhieuXuatHangHoaChiTiet::where('id', $id)->value('SoLuong');
            $SoLuongTon = DanhMucHangHoa::where('MaHang', $request->MaHang)->value('SoLuongTonDau');
            if (((int)$request->SoLuong - (int)$SoLuong_in_PXHHCT) - (int)$SoLuongTon > 0) {
                return response()->json(['error'], 400);
            } else {
                //Lấy Thuế GTGT trong cơ sở dũ liệu để so sánh
