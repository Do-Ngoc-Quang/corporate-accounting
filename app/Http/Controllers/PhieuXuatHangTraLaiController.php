...
use App\Models\PhieuXuatHangTraLai;
use App\Models\PhieuXuatHangTraLaiChiTiet;


class PhieuXuatHangTraLaiController extends Controller
{
    // ------------------------------------------------------------------------------------------------------------------------- //
    public function index()
    {
        $phieuxuathangtralai = PhieuXuatHangTraLai::get();
        return view('content.giaodich.phieuxuathangtralai', compact('phieuxuathangtralai'));
    }

    // Lấy data của Phiếu xuất để hiển thị lên màn hình
    public function get_phieuxuathangtralai($machungtu)
    {
        $phieuxuathangtralai = PhieuXuatHangTraLai::where('MaChungTu', $machungtu)->get();
        return response()->json($phieuxuathangtralai);
    }

    //Hàm xử lý tạo mới 
    public function store(Request $request)
    {
        $exist_phieuxuathangtralai = PhieuXuatHangTraLai::where('MaChungTu', $request->MaChungTu)->value('MaChungTu');

        //Kiểm tra xem đã tồn tại trong database chưa
        if (!$exist_phieuxuathangtralai) {
            ...
            $validator = Validator::make($request->all(), [
                ...
                'TaiKhoanNoGiaMua' => 'required',
                'TaiKhoanCoGiaMua' => 'required',
            ...
            PhieuXuatHangTraLai::create([
                ...
                'TaiKhoanNoGiaMua' => $request->TaiKhoanNoGiaMua,
                'TaiKhoanCoGiaMua' => $request->TaiKhoanCoGiaMua,
            ...
            PhieuXuatHangTraLaiChiTiet::create([
                'MaChungTu' => $request->MaChungTu,
                ...
                'DonGiaMua' => $request->DonGiaMua,
                'ThanhTienGiaMua' => $request->ThanhTienGiaMua,
     ...
     {
         $phieuxuathangtralai = PhieuXuatHangTraLai::find($id);

         if ($phieuxuathangtralai) {
             ...
             // //Cập nhật số dư nợ cho tài khoản GTGT ghi nhận có
             // $taiKhoanNoGTGT = DanhMucTaiKhoan::find($id_TaiKhoanCoGTGT);
             // $taiKhoanNoGTGT->update([
             //     'SoDuCoDau' => $temp_SoDuCoDau_GTGT
             // ]);
     ...
     public function get_phieuxuathangtralaichitiet($machungtu)
     {
         $phieuxuathangtralaichitiet = PhieuXuatHangTraLaiChiTiet::where('MaChungTu', $machungtu)->get();
         return response()->json($phieuxuathangtralaichitiet);
     }

     //Hàm xử lý cập nhập chi tiết (bảng phụ)
     public function phieuxuathangtralaichitiet_update(Request $request, $id)
     {
         $phieuxuathangtralaichitiet = PhieuXuatHangTraLaiChiTiet::find($id);

         if ($phieuxuathangtralaichitiet) {
             ...
             // Cập nhật chi tiết
             $phieuxuathangtralaichitiet->update([
                 'DonViTinh' => $request->DonViTinh,
                 'SoLuong' => $request->SoLuong,
                 'DonGiaVon' => $request->DonGiaVon,
                 'ThanhTienGiaVon' => $request->ThanhTienGiaVon,
