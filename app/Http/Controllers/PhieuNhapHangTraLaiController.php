...
use App\Models\PhieuNhapHangTraLai;
use App\Models\PhieuNhapHangTraLaiChiTiet;

class PhieuNhapHangTraLaiController extends Controller
{
    // ------------------------------------------------------------------------------------------------------------------------- //
    public function index()
    {
        $phieunhaphangtralai = PhieuNhapHangTraLai::get();
        return view('content.giaodich.phieunhaphangtralai', compact('phieunhaphangtralai'));
    }

    // Lấy data của Phiếu xuất để hiển thị lên màn hình
    public function get_phieunhaphangtralai($machungtu)
    {
        $phieunhaphangtralai = PhieuNhapHangTraLai::where('MaChungTu', $machungtu)->get();
    ...
    {
        $exist_phieunhaphangtralai = PhieuNhapHangTraLai::where('MaChungTu', $request->MaChungTu)->value('MaChungTu');

        //Kiểm tra xem đã tồn tại trong database chưa
        if (!$exist_phieunhaphangtralai) {
            ...
            PhieuNhapHangTraLaiChiTiet::create([
                'MaChungTu' => $request->MaChungTu,
    ...
    {
        $phieunhaphangtralai = PhieuNhapHangTraLai::find($id);

        if ($phieunhaphangtralai) {
    ...
    public function get_phieunhaphangtralaichitiet($machungtu)
    {
        $phieunhaphangtralaichitiet = PhieuNhapHangTraLaiChiTiet::where('MaChungTu', $machungtu)->get();
        return response()->json($phieunhaphangtralaichitiet);
    }

    //Hàm xử lý cập nhập chi tiết (bảng phụ)
    public function phieunhaphangtralaichitiet_update(Request $request, $id)
    {
        $phieunhaphangtralaichitiet = PhieuNhapHangTraLaiChiTiet::find($id);

        if ($phieunhaphangtralaichitiet) {
            ...
            // Cập nhật chi tiết
            $phieunhaphangtralaichitiet->update([
                'DonViTinh' => $request->DonViTinh,
                'SoLuong' => $request->SoLuong,
                'DonGiaVon' => $request->DonGiaVon,
                'ThanhTienGiaVon' => $request->ThanhTienGiaVon,
